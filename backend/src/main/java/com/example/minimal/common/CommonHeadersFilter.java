...
public class CommonHeadersFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain chain)
		...
		String traceId = Optional.ofNullable(req.getHeader(ApiHeaders.TRACE_ID)).filter(s -> !s.isBlank())
				.orElseGet(IdGenerator::newId);

		// ② Idempotency-Key（任意）取得
		String idmpKey = Optional.ofNullable(req.getHeader(ApiHeaders.IDEMPOTENCY_KEY)).filter(s -> !s.isBlank())
				.orElse(null);
