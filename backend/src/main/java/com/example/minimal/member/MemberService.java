...
public class MemberService {
	...
	public MemberService(
			MemberRepository memberRepository, IdempotentRequestRepository idemRepo, ObjectMapper objectMapper,
	...
	public MemberResponse create(CreateMemberRequest req, String idempotencyKey) {
		...
		final String requestHash = StringUtils
				.sha256(code + "|" + name + "|" + StringUtils.safe(email) + "|" + StringUtils.safe(note));
			...
			try {
				...
				if (claimed == null) {
					throw new UnexpectedPersistenceException(null, ErrorMessage.COM_SERVER_ERROR_MESSAGE,
							ErrorCode.COM_SERVER_ERROR, null);
				}
				...
				return idempotencyFetchTxTemplate.execute(_ -> {
					var hit = idemRepo.findByEndpointAndIdempotencyKey(endpoint, normalizedIdempotencyKey)
							.orElseThrow(() -> new UnexpectedPersistenceException(null,
									ErrorMessage.COM_SERVER_ERROR_MESSAGE, ErrorCode.COM_SERVER_ERROR,
									duplicate.getCause()));
					if (!hit.getRequestHash().equals(requestHash)) {
						throw new IdempotencyConflictException(ApiHeaders.IDEMPOTENCY_KEY,
								ErrorMessage.IDE_DEFFERENT_REQUEST_MESSAGE, ErrorCode.IDE_VAL_DEFFERENT_REQUEST);
					}
					if (hit.getMemberId() == null) {
						throw new IdempotencyConflictException(ApiHeaders.IDEMPOTENCY_KEY,
								ErrorMessage.IDE_SAME_KEY_RUNNING_MESSAGE, ErrorCode.IDE_VAL_SAME_KEY_RUNNING);
					}
					MemberEntity memberEntity = memberRepository.findByIdAndDeletedAtIsNull(hit.getMemberId())
							.orElseThrow(() -> new UnexpectedPersistenceException(null,
									ErrorMessage.COM_SERVER_ERROR_MESSAGE, ErrorCode.COM_SERVER_ERROR,
									duplicate.getCause()));
		...
		if (memberRepository.existsByCodeAndDeletedAtIsNull(code)) {
			throw new DuplicateValueException(Fields.code, ErrorMessage.MBR_CONFLICT_CODE,
					ErrorCode.MBR_VAL_CONFLICT_CODE);
		}
		...
		if (normalizedIdempotencyKey != null) {
			var ir = idemRepo.findByEndpointAndIdempotencyKey(endpoint, normalizedIdempotencyKey).orElse(null);
			if (ir != null) {
